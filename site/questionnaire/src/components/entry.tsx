...
const defaultDatetimePickerGroup = [
  { items: initYears },
  { items: initMonthes },
  { items: initDays() },
];

const getDatetimeSelected = (value: string) => {
  let arr = [];
  ...
  for (let n = 0; n < dateArr.length; n++) {
    for (let i = 0; i < defaultDatetimePickerGroup[n].items.length; i++) {
      if (
        "" + dateArr[n] ===
        defaultDatetimePickerGroup[n].items[i].label + ""
      ) {
        arr.push(i);
        break;
      }
...
export default ({ data, updateHandler }) => {
  ...
  const [sexPickerGroup, setSexPickerGroup] = useState([
  ...
  const [datetimePickerSelected, setDatetimePickerSelected] = useState(
    getDatetimeSelected(currentDay)
  );
  const [datetimePickerGroup, setDatetimePickerGroup] = useState(
    defaultDatetimePickerGroup
  );
      ...
      <PanelBody>
        ...
        <Picker
          ...
          onChange={(selected) => {
            ...
            selected.forEach((s, i) => {
              value +=
                (!value ? "" : "-") + datetimePickerGroup[i]["items"][s].label;
            });

            setDatetimePickerValue(value);
            setDatetimePickerShow(false);
          }}
          onGroupChange={(item, i, gi, selected, instance) => {
            let days = getDaysByMonthAndYear(
              initYears[selected[0]].label,
              initMonthes[selected[1]].label
            );

            setDatetimePickerGroup([
              { items: initYears },
              { items: initMonthes },
              { items: days },
            ]);
